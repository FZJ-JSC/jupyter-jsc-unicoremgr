...
def _jd_add_input_files(config, jhub_credential, initial_data, jd, logs_extra={}):
    ...
    service_ = initial_data["user_options"]["service"].rstrip("/").replace("/", "_")
    service_to_skip = [
        f"{x}_"
        for x in config.get("systems", {})
        .get("mapping", {})
    ...
    skip_prefixs.extend(service_to_skip)
    system_to_skip = [
        f"{x}_"
        for x in config.get("systems", {})
        .get("mapping", {})
    ...
    for filename in input_files:
        ...
        if filename.startswith(f"{service_}_"):
            newname = filename[len(service_) + 1 :]
